...
fn test_initialize_succeeds() {
    ...
    // println!("data length {:?}", raw_plan_account.data.len());
    // println!(
    //     "MP discriminator length {:?}",
    //     MeditationPlan::DISCRIMINATOR.len()
    // );
    // let plan = MeditationPlan::try_from_slice(&raw_plan_account.data.as_slice()).unwrap();
